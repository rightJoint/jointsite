...
trait RoutesCollection_Modules_Services
{
    //GET: /siteman/services
    static function getRoute_ModuleServices($routes_ns):JointSiteRoute
    {
        $route = (new JointSiteRoute())
            ->withController('JointApp\Controllers\Components\Controller_Components_Services',
            //array('processUri' => '/test/migrations/migrationsList')
            )
            ->withModel('JointApp\Models\Components\Model_Components_Services');

        //GET: /siteman/services.../listview
    ...
    //POST: /siteman/users
    static function postRoute_ModuleServices($routes_ns):JointSiteRoute
    {
        $route = (new JointSiteRoute())
            ->withController('JointApp\Controllers\Components\Controller_Components_Services',
            )
        ...
        //POST: /siteman/services..listview
        if (!isset($routes_ns[3]) or strtolower($routes_ns[3]) == 'listview') {
            $route
        ...
        //POST: /siteman/services/editview
        elseif (strtolower($routes_ns[3]) == 'editview') {
            $route
                ->withAction('postEditView')
                ->withView('JointApp\Views\Modules\ModuleEditView');
        }
        //POST: /siteman/services/deleteview
        elseif (strtolower($routes_ns[3]) == 'deleteview') {
            $route
                ->withAction('postDeleteView')
                ->withView('JointApp\Views\Modules\ModuleEditView');
        }
        //POST: /siteman/services/newview
        elseif (strtolower($routes_ns[3]) == 'newview') {
            $route
