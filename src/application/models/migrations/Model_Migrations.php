...
class Model_Migrations extends RecordsModel
{
    ...
    function glob_migration_files()
    {
        ...
        */
        foreach (glob(JOINT_SITE_REQUIRE_DIR."/migrations/*.sql") as $mirgation_file) {
    ...
    function copyCustomFields():bool
    {
        if($this->recordStructureFields->record["migration_name"]["curVal"] ){
            if(file_exists(JOINT_SITE_REQUIRE_DIR."/migrations/".
                $this->recordStructureFields->record["migration_name"]["curVal"])){
                $commands = $this->parse_sql_file(JOINT_SITE_REQUIRE_DIR."/migrations/".
                    $this->recordStructureFields->record["migration_name"]["curVal"]);
    ...
    function updateMigration($req_arr):bool
    {
        ...
        file_put_contents(JOINT_SITE_REQUIRE_DIR."/migrations/".$this->recordStructureFields->record["migration_name"]["curVal"].".sql", $commands);
    ...
    function updateCustomFields():bool
    {
        ...
        if($_POST){
            ...
            or !isset($this->recordStructureFields->record["commands"]["curVal"])){
                file_put_contents(JOINT_SITE_REQUIRE_DIR."/migrations/".$this->recordStructureFields->record["migration_name"]["curVal"], $commands);
