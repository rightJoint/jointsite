...
class View_JointSite_About_Lang extends View_JointSite_About
{
    ...
    protected static function pageContentJointSite(\stdClass $contentBlock, string $langSl = ''):string
    {
        return '<section>'.
            ...
            'но болле затратные с точки зрения регистрации и поддержки нескольких доменов. '.
            ...
            'Мультиязычный текст выносился в отдельные файлы и виде /stdClass подключался к view. '.
            'Приложение разбирает Uri запроса, и определяет язык, например русский, английский, или язык по умолчанию. '.
            '<ul>'.
            '<li>Для загрузки языковых файлов из классов потребуется именя класса в таком регистре Ru, En (landNs)</li>'.
            '<li>Для постановки в массивы или запросы в базу данны нижнем регистре ru, en (landLw)</li>'.
