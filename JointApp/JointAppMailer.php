...
class JointAppMailer
{
    public PHPMailer $phpMailer;
    public Model_Pdo $model_Pdo;
    /*

*/
    public function __construct($phpMailer, Model_Pdo $model_Pdo)
    {
        $this->phpMailer = $phpMailer;
        $this->phpMailer->isSMTP();
        $this->phpMailer->isHTML(true);
        $this->model_Pdo = $model_Pdo;
    }
    ...
    public function addNotification(string $templateName,
        //user, group, email
                                    string $subscriber_type,
                                    string $type_id,
        //$template_params string format json decode
                                    ...
                                    $method):bool
    {
        ...
        //$findTemplate_res = $this->pdoQuery($findTemplate_qry);
        if($findTemplate_res = $this->model_Pdo->pdoQuery($findTemplate_qry) and $findTemplate_res->rowCount() === 1){
            $findTemplate_row = $findTemplate_res->fetch(\PDO::FETCH_ASSOC);
            $newNtf_id = $this->model_Pdo->createGUID();
            ...
            if($this->model_Pdo->pdoQuery($addNtf_qry)){
                if($send_now){
    ...
    public function SendNtf(string $ntf_id = '', $method = '')
    {
        ...
        $sendList_res = $this->model_Pdo->query($sendList_qry);
        ...
        if($sendList_res->rowCount()){
            while ($sendList_row = $sendList_res->fetch(\PDO::FETCH_ASSOC)){
                ...
                if($sendList_row['subscriber_type'] == 'user'){
                    ...
                    $groupUsers_res = $this->model_Pdo->pdoQuery($groupUsers_qry);
                    ...
                    $template_res = $this->model_Pdo->pdoQuery($template_qry);
                ...
                $log_json = json_encode($send_res, true);

                $updateNtfList_qry = "update ntfList_dt set ".
                    "send_date='".date("Y-m-d H:i:s")."', ".
                    "send_res=".$res_text.", ".
                    "send_log = '".$log_json."' "."where ntf_id='".$sendList_row["ntf_id"]."'";
                $this->model_Pdo->query($updateNtfList_qry);
            }
        }
    }

    function sendNtfForUser($ntf_id, $user_id):array
    {
        $return = array(
            "put_ntf" => 0,
            "send_flag" => 0,
        ...
        if($template_res->rowCount() === 1){
            ...
            $findUserNtf_flag_res = $this->model_Pdo->pdoQuery($findUserNtf_flag_qry);
            ...
            if($findUserNtf_flag_res->rowCount() === 1){
                ...
                $putNtf_qry = "insert into ntfRead_dt (ntf_id, user_id, read_date, put_date, send_flag) ".
                    ...
                    $send_flag.
                    ")";
                $this->model_Pdo->pdoQuery($putNtf_qry);
