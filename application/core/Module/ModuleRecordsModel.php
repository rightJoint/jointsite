...
class ModuleRecordsModel extends RecordsModel
{
    ...
    function __construct()
    {
        ...
        $this->lang_map["file_err"] = array(
            "unlink_err" => array(
                "en" => "unlink error in Controller_User",
                "rus" => "ошибка при удалении в Controller_User",
            ),
            ...
            "mvf_err_load" => array(
                "en" => "load error in Controller_User",
                "rus" => "ошибка при загрузке в Controller_User",
            ),
            "form_h2" => array(
                "en" => "main info",
        ...
        if(!$_FILES[$field_name]["error"]){
            ...
            if(strpos(" ".$this->editFields[$field_name]["file_options"]["accept"], $file_extension)){
                ...
                $moved = @move_uploaded_file($_FILES[$field_name]['tmp_name'], $_SERVER["DOCUMENT_ROOT"].
                    $this->editFields[$field_name]["file_options"]["load_dir"]."/".$file_name.".".$file_extension);
                ...
                $this->log_message .= $this->lang_map["file_err"]["mvf_err_extension"][$_SESSION["lang"]];
                return false;
            }
        }else{
    ...
    function deleteRecordFetchFile($field_name)
    {
        if($this->record[$field_name]["fetchVal"]){
            if(@unlink($_SERVER["DOCUMENT_ROOT"].
                $this->editFields[$field_name]["file_options"]["load_dir"]."/".$this->record[$field_name]["fetchVal"])){
