...
class ntSendModel extends Model_pdo
{
    ...
    public function SendNtf($ntf_id, $method)
    {
       // echo "SendNtf==1<br>";
        ...
        if($sendList_res->rowCount()){
          //  echo "SendNtf? rowCount=".$sendList_res->rowCount()."<br>";
    ...
    function sendNtfForUser($ntf_id, $user_id)
    {
        ...
        if($template_res->rowCount() === 1){
            //echo "sendNtfForUser: rowCount==1<br>";
            //exit;
            $template_row = $template_res->fetch(PDO::FETCH_ASSOC);
            if($template_row["subscriber_type"] == "group"){
                //echo "sendNtfForUser: subscriber_type==group<br>";
                //exit;
                ...
                //echo "sendNtfForUser: subscriber_type==user<br>";
                //exit;
            ...
            if($findUserNtf_flag_res->rowCount() === 1){
                ...
                //echo "sendNtfForUser: findUserNtf_flag_res=rowCount==1<br>";
                //exit;
                ...
                if($findUserNtf_flag_row["send_ntf"]){
                    //echo "sendNtfForUser: send_ntf-Ok<br>";
                    $return["send_flag"] = 1;
        ...
        можете отследить статус вашей заявки по ссылке https://".$_SERVER["HTTP_HOST"]."/applications/details/".
                ...
                //echo "sendNtfForUser: insert ntfRead_dt<br>";
                //exit;
