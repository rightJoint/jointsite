...
class applicationsView extends View
{
    function __construct()
    {
        ...
        $this->scripts[] = "/js/applications/defaultForm.js";
        $this->styles[] = "/css/applications/detail.css";
    }
    ...
    function print_page_content()
    {
        if(is_array($this->view_data["basket"])){
            foreach ($this->view_data["basket"]["prod"] as $prodAlias=>$prodInfo){
                $prodCost = $prodInfo["count"]*$prodInfo["cardPrice_".$_SESSION["lang"]];
                $srvApplBasket.= "<div class='appl-basket-line'>".
                    $prodInfo["cardName_".$_SESSION["lang"]]." =".$prodInfo["count"];
                $srvApplBasket.= "(".$prodInfo["unit_".$this->view_data["basket"]["lang"]].")";
                $srvApplBasket.= "* ".$prodInfo["cardPrice_".$_SESSION["lang"]]." = ".$prodCost." ".$prodInfo["cardCurr_".$_SESSION["lang"]].
                    "</div>";
            }
        }else{
        ...
        echo
            ...
            "<div class='cid-phone'><img src='/img/Services/typeNum.png'><span>".$this->view_data["rd"]["clientPhone"]["curVal"]."</span></span></div>".
            "</div>".
            "</div>".
        ...
        if($this->view_data["rd"]["clientSubject"]["curVal"]){
        ...
        if($_SESSION["site_user"]['user_id']){
            $return.="<img src='".USERS_AVATARS_DIR."/".$_SESSION["site_user"]['avatar']."'>".$_SESSION["site_user"]['accAlias'].
        ...
        if($this->view_data["rdCm"]["err"]["content"]){
